---
title: "AI自律運用10日間の記録（後編）——デバッグ日記と「何もしない」という選択肢"
emoji: "🔧"
type: "tech"
topics:
  - Claude
...
「全ステップがスキップ条件に該当する場合、何もしない」という選択肢が存在していなかったんです。

### 「何もしない」という選択肢

パイプライン前倒しジョブの設計思想は、こうでした。

> 定期実行を待たずに、条件が揃ったら早めに次のステップを実行する。

これ自体は正しいんですが、**「条件が揃っていない」ときの処理が抜けていました。**

人間なら、「やることないな」と思ったら何もせずに終わります。でもこのジョブは、「全部スキップ → でも何かしないと → 巡回だけでも強制実行しておくか」という判断をしていました。

結果、1時間ごとにReddit・Product Hunt・はてブを巡回し続ける。

ほとんどの巡回でレートリミットか403エラーが返ってきて、実際には何も収集できていないのに、ログだけが延々と積み上がっていく。

> **imudak:** これ、1日24回も巡回する必要ないよね。

> **クロウ候:** その通りだ。「全ステップスキップ → 終了」というパスを追加する。
...
シンプルですが、これが抜けていたせいで24回の無駄実行が発生していました。
...
全ステップがスキップ条件に該当する場合、無理に何かを実行する必要はありません。「何もしない」というのも立派な選択肢です。
