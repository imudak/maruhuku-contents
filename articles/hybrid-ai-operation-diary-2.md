...
復旧後、Claude Code（ローカルのAI）にインシデントレポートを書いてもらい、クロウ候に引き継いで記憶させました。次に同じことが起きたとき、同じ失敗を繰り返さないために。
...
### Notionが空でも、プロジェクト自体は動いている

全プロジェクトのNotionページ本文が空だった話ですが、実は各プロジェクトにはちゃんと仕様書（steering/VISION.md）が作られていて、AI側は何を作るか分かっていました。困るのは私だけ。Notionを見ても何のプロジェクトか分からない。

これは「仕様書をNotionにも反映するフロー」が抜けていたという、単純なフローのバグでした。
