...
- ターミナルを閉じるとプロセスも全部終了します
- PCを再起動すると作業状態は消えます

### tmuxの場合

- `Ctrl+b d`でデタッチするとプロセスはバックグラウンドで継続します
- ターミナルを閉じても`tmux attach`で元の状態に戻れます
- 長時間かかるビルドやサーバーを動かしたまま離席できます

この違いは実際に使ってみると想像以上に大きいです。

## tmuxが特に役立つ場面

1. **SSHでリモートサーバーに接続している時** - 接続が切れても作業が失われません
2. **長時間のタスク実行中** - ターミナルを閉じても処理は継続します
3. **ペアプログラミング** - 同じセッションを複数人で共有できます

## Windows Terminal + WSL + tmuxの組み合わせ

この3つの組み合わせは相性が良いです。

- SSHやWSLのセッションが切れてもtmuxセッションは生き続けます
- Windows Terminalを閉じても、再度`tmux attach`で作業を再開できます
- 複数のペイン（分割画面）を1つのタブで管理できます
