...
将棋の棋譜解説動画を自動生成したくて、**Remotion**を使ってみました。ReactコンポーネントでMP4/WebMを生成できるフレームワークです。

この記事では、Remotionのセットアップから実践的なパターンまでをまとめます。実際に開発した棋譜解説動画の自動生成システム（mirushogi）での知見も含めています。
...
コンポーネント内でハードコードせず、常にこのフックから値を取得すれば、解像度やFPS変更への耐性が高い設計になります。
...
シーンの長さが散在するとメンテナンスが大変になります。定数で一元管理すると楽です。
...
CLIだけでなく、Node.js APIから直接レンダリングできます。これにより、Webサーバーやバッチ処理システムに組み込めます。
...
## 8. 実践例: 将棋の棋譜解説動画を自動生成（mirushogi事例）

ここからは、実際に開発した**mirushogi**（将棋の棋譜解説動画を自動生成するシステム）での実践知見をまとめます。
...
Remotionを実際に使う中で遭遇した問題と解決策をまとめます。
...
:::message
**`<Audio>`のインポート元について:** Remotion v4以降、`<Audio>`は`@remotion/media`パッケージからインポートすることが推奨されています。`remotion`パッケージの`<Audio>`（現在は`<Html5Audio>`）も動作しますが、`@remotion/media`版はレンダリング時により正確に音声を抽出します。
:::
...
Remotionは活発に開発が続いており、以下の動きがあります。

- **Remotion Lambda**の最適化（分散レンダリングのコスト削減）
- **Remotion Studio**の進化（Editor Starterテンプレートも公開済み）
- **ブラウザ内レンダリング**（WebCodecs / Mediabunnyとの統合）
- **AI連携**（公式のLLMシステムプロンプトも提供されている）

「動画をコードで書く」というアプローチは、定型動画の大量生成との相性がとても良いと感じました。
