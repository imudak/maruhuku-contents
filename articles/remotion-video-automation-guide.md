...
動画コンテンツの需要が増加する中、「動画を手作業で編集する」というワークフローに限界を感じているエンジニアも多いと思います。

**Remotion**は、Reactコンポーネントで動画を記述し、プログラマティックにMP4/WebMを生成できるフレームワークです。つまり、UIを作るのと同じ感覚で動画を作れます。

本記事では、Remotionの基礎から実践的なパターンまでを解説します。筆者が実際に開発した将棋の棋譜解説動画を自動生成するシステム（mirushogi）での知見も交えながら、「動画をコードで生成する」世界を紹介します。
...
ここからは、筆者が実際に開発した**mirushogi**（将棋の棋譜解説動画を自動生成するシステム）での実践知見を紹介します。
...
Remotionを実プロダクトで使う中、遭遇した問題と解決策をまとめます。
...
Remotionで動画自動生成パイプラインを構築してみてはいかがでしょうか。
