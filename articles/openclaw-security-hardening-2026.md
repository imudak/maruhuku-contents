...
外部からgatewayに直接接続する必要はなかったので、`loopback`に変更しました。loopbackにすると、同じマシン内からしかgatewayに接続できなくなります。仮にgatewayトークンが漏洩しても、外部のマシンからは接続できません。
...
動かなくなった原因は単純でしたが、なぜこんなに急にセキュリティが厳しくなったのかが気になります。エージェントにCHANGELOGや関連記事を調べさせたところ、2026年2月のOpenClaw周辺はかなり騒がしい状況でした。
...
forkではなくprivateミラーとして保持することにしました。forkだとGitHub上で関係が見えてしまうためです。直接的なセキュリティ対策ではありませんが、セキュリティ修正が必要なときに「本家リポジトリが消えていてパッチが当てられない」という事態を防ぎます。
...
手動更新は忘れます。CVE-2026-25253のような脆弱性が公開されたとき、パッチ適用が1日遅れるだけでリスクにさらされます。cronで毎朝バージョンを比較して、差分があれば`gateway update.run`を実行するようにしました。
...
v2026.2.19以降はデフォルトでトークンが自動生成されますが、念のため`token`が設定されていることを確認しました。`none`だとgatewayのWebSocket URLさえわかれば誰でも接続できてしまい、設定変更やコマンド実行が自由にできます。前述の3万台の露出問題と直結する設定です。
...
自分の場合は、OpenClaw専用のminiPCを1台用意して、普段使いのマシンとは分離しています。物理的なサンドボックスです。万が一侵害されても、仕事の書類や個人の写真といった本当に大事なデータには届きません。ソフトウェア的な対策と合わせて安心感があります。こういう割り切りも選択肢の1つだと思います。

ちょうど冬季オリンピックを見ていて思ったのですが、スキーをやっていたら巨大なジャンプ台を作りたくなって作ってしまう、スノーボードで高く飛んだら楽しかったからスポーツにしてしまう。周囲がどう見ても危ないと言っても、面白いから始めてしまう人たちがいて、そこで当然のように怪我人が出て、そしてすぐにルールや安全装置を整備する人たちも出てくる。欧米のテック界隈もまったく同じ構造だと感じます。

OpenClawもそうで、ローカルマシンにフルアクセスできるAIエージェントなんて、冷静に考えれば危ないに決まっています。でも面白いから作る人がいて、使う人がいて、攻撃する人がいて、直す人がいる。結果として世界は更新前より安全になっていないし安定もしていないのですが、どこか1歩前には進んでいる。そういうダイナミズムに乗っかっているのだという自覚は持っておきたいと思います。
...
| 対策 | 防ぐ攻撃 |
|------|----------|
| バージョン更新 | CVE-2026-25253（リンク1つでRCE）、シェルインジェクション等 |
| gateway.bind → loopback | トークン漏洩時の外部からの接続 |
| ミラーリング | リポジトリ消失時のパッチ適用不能 |
| 自動更新 | パッチ適用の遅延による被害拡大 |
| gateway.auth.mode = token | 認証なしgatewayへの不正接続 |
| 専用miniPCで分離 | 侵害時の被害範囲（本業データへの到達） |

OpenClawを使っている方は、まず`openclaw --version`でv2026.1.29以上であることを確認してください。前述のCVE-2026-25253（トークン窃取→任意コード実行）の修正が入っていないバージョンは、細工されたリンクを1つクリックするだけで侵害される可能性があります。

## 後日談

このセキュリティ強化アップデートが引き起こした別の問題について、続編を書きました。
