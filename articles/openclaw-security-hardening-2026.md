...
v2026.2.19以降はデフォルトでトークンが自動生成されますが、念のため`token`が設定されていることを確認しました。`none`だとgatewayのWebSocket URLさえわかれば誰でも接続できてしまい、設定変更やコマンド実行が自由にできます。3万台の露出インスタンスの多くがこの状態だったようです。
