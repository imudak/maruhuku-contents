...
## 「storage/specs/ に置けって言ったのに、なんで steering/features/ に置くの？」

Claude Codeに機能追加を依頼するたびに、仕様ファイルが違う場所に生成される。ある時は `storage/specs/`、次は `steering/features/`、たまに `docs/` にも。

注意すると直るけど、次の機能ではまた違う場所に置く。CLAUDE.mdに「MUSUBIを使え」と書いてあるので、Claude Codeは「ああMUSUBIね、はいはい」と認識しているのですが、具体的にどこに何を置くかは **毎回忘れる** わけです。

これは記憶の問題ではなく、設計の問題でした。
