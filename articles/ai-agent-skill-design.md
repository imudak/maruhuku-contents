---
title: "AIエージェントにスキルを持たせる — 「わかったフリ」を防ぐ設計パターン"
emoji: "🧠"
type: "tech"
topics:
  - AI
...
## AIエージェントは「わかったフリ」の天才

AIコーディングエージェント（Claude Code、GitHub Copilot等）に仕事を任せていると、ある日気づきます。

**「こいつ、わかってないのに作業を進めている」**

たとえば、私がMUSUBI（仕様駆動開発フレームワーク）を使ったプロジェクトでClaude Codeに機能追加を依頼した時のこと。CLAUDE.mdにはMUSUBIの設定が書いてあるので、Claude Codeは「ああMUSUBIね、はいはい」という顔をして作業を始めました。

結果、仕様ファイルの配置場所は自己流、Change Managementの仕組みは完全スルー、テストのトレーサビリティもガン無視。**表面的にそれっぽいファイルを作っているだけ** でした。

これは、エージェントに「知識」はあっても「手順」がないからです。
...
「AIに仕事を任せたら雑な結果が返ってきた」という経験がある人は、スキルの導入を検討してみてください。AIは知識を持っています。足りないのは、あなたのプロジェクト固有の手順です。
