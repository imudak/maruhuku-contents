...
jjの基本概念、Gitとの違い、そしてGitリポジトリとの連携方法についてまとめました。
...
jjの最大の特徴は、説明を何度でも書き直せることです。Gitの `commit --amend` よりシンプルに使えます。
...
1. コミットを忘れない - ファイルを保存すれば自動で記録されるので、作業に集中できる。Gitで「あ、コミット忘れた」となることがなくなった

2. 説明は後で考えればいい - コーディング中は `jj new` だけ実行して、後でまとめて `jj desc` で説明を書ける。思考の流れが途切れない

3. typoの修正が楽 - コミットメッセージに誤字を見つけても、`jj desc` で即座に直せる。Gitの `commit --amend` より圧倒的に簡単

### 気になった点

1. 学習コスト -「変更（change）」と「コミット（commit）」の概念の違いを理解するのに少し時間がかかった

2. エディタとの統合 - VSCodeなどの統合はGitほど成熟していない。コマンドラインでの操作が基本

3. チーム開発での運用 - 個人では便利だが、チームで使うには運用ルールをしっかり決める必要がある

## まとめ

jjは「**きれいな履歴を作る**」ことを目指して設計されたバージョン管理システムです。

個人開発では、コミットメッセージを何度も書き直したり、履歴を自由に整理できる柔軟性が便利でした。特に試行錯誤の多い開発フェーズで、`git add` と `git commit` の手間が省ける点は想像以上に快適です。

既存のGitリポジトリでも `jj git init --colocate` すればすぐに試せます。

jjの設計思想やワークフローの詳細については、以下の記事にまとめています。
