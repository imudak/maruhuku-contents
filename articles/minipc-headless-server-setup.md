...
ミニPCを箱から出してすぐにリモート接続はできません。リモートデスクトップを有効化するために、最初の1回だけ以下のどちらかが必要になります。

### 方法1: テレビをモニター代わりにする

家にあるテレビとHDMIで接続し、100均で買った有線マウス（300〜500円程度）で操作します。リモートデスクトップを有効化したら、マウスは引き出しにしまって終わりです。

### 方法2: 出荷時設定に賭ける

運が良ければ、メーカー出荷時にリモートデスクトップが有効化されている場合があります。ミニPCをLANケーブルで接続し、別のPCから接続を試してみてもよいかもしれません。ただし、ほとんどの場合は無効になっています。
...
**HDMIダミープラグ**をミニPCのHDMI端子に挿しておきます。Amazonなら数百円で買えます。これを挿すことで、ミニPCは「モニターが接続されている」と認識し、正常な解像度で動作します。
...
RDP経由だと設定の反映がうまくいかない場合は、レジストリを直接修正します。
...
- ルーターのポート開放が不要
- 固定IPが不要
- IPA（情報処理の推進機構）とNTT東日本の共同開発で信頼性が高い
...
ミニPCがスリープに入ると外から繋げなくなります。Windowsの電源設定で「スリープなし」にしておく必要があります。
...
次回は、このミニPC上でWSL2を動かし、外部からSSH接続できるようにする方法をまとめる予定です。
