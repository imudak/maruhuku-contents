...
「1つの変更を育てていく」という考え方は、以下のような転換をもたらします。
...
# 5. PR作成（明示的なブランチ名を指定）
jj git push -b feature/auth-123  # --branch の省略形
...
jj git push -b feature/auth-123

# 9. さらに修正があれば繰り返す
# 最終的にpushされるのは1つのchange
```

補足: `jj git push -c @`（`--change @`の省略形）では、現在のchange（@）に対して自動的にブックマーク名が生成されます（デフォルトは`push-<change_id>`形式）。このブックマークは自動的にtrackされ、2回目以降では同じブックマークが更新されます。一方、`-b`（`--branch`の省略形）では明示的なブランチ名を指定し、PR用のブックマークとして扱います。
...
jjでは、**jjコマンドを実行するたびに、working copyのスナップショットが自動的に記録されます**。`jj desc`でコミットメッセージを書く必要すらありません。
...
1. **jjコマンド実行時にworking copyのスナップショットが自動的に作られる**（ファイルシステムを常時監視しているわけではなく、`jj status`等のコマンド実行がトリガーです）
...
| ファイル編集は記録されない | **jjコマンド実行時にスナップショットを自動記録** |
...
jjは単なる「Gitの代替ツール」ではなく、**コードレビューとバージョン管理に対する異なる考え方**を持ったツールです。
...
これらを体験すると、`fix typo`コミットを何十個も積み重ねていた頃には戻りにくくなりました。changeを育てるというアプローチは、自分にとってバージョン管理の考え方が変わるきっかけになりました。
