---
title: 技術ブログをGitHubリポジトリからpushで自動投稿する方法
emoji: 🚀
type: tech
topics:
  - GitHub
...
技術記事を書くとき、「ローカルで書いてGitHubにpushしたら、自動で各プラットフォームに投稿されたら楽なのに」と思い、調べてみました。

Zenn・Qiita・noteの3つのプラットフォームについて、GitHubリポジトリとの連携による自動投稿が可能かどうかをまとめます。
...
- 自動投稿する公式な手段は提供されていない

### 非公式な方法

以下のような非公式な方法は存在しますが、使わない方がいいと考えています。
...
- 利用規約違反のリスクがある
- ビジネス運用やブランドアカウントでは特にリスクが高い
- メンテナンスコストが高い

### 実際の運用方法

noteについては、以下のワークフローで運用しています。

1. GitHubリポジトリでMarkdownとして記事を管理
2. 投稿時にnoteのエディタに手動でペースト
3. note固有の装飾（見出し画像など）を追加して公開

これで記事のバックアップと版管理はできつつ、規約リスクを避けられます。

## 複数プラットフォームを一元管理する方法

ZennとQiitaの両方に投稿したい場合、リポジトリ構成を工夫すると管理しやすくなります。
...
GitHubリポジトリでの記事管理は、技術記事を長期的に運用していく上で手堅い方法です。ZennとQiitaは公式サポートがあるので、自分はこの構成で運用しています。
