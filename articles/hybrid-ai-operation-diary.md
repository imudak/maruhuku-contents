---
title: "AI自律運用10日間の記録（前編）——5日でプロジェクト20個、失敗もセットで"
emoji: "📔"
type: "tech"
topics:
  - Claude
...
こういう単純作業をAIに任せられるなら、同じ要領で他のサービスの設定も全部やらせられるな、と思いました。
...
制御を気にしないと大変なことになるな……と、このときはまだ軽く考えていました。
...
クロウ候が `config.patch` で修正を進めました。存在しないモデルを設定から消す、それだけのはずでした。

### シークレット消失→Discord断絶

ところが、設定変更の過程で思わぬ副作用が発生。Discordのボットトークン、ゲートウェイの認証トークン、Web検索のAPIキー——**シークレット系の値が全て消えました。**

バックアップファイル（`.bak`）と現在のファイルの更新間隔はわずか12秒。人間が操作するには短すぎる。OpenClawの自動マイグレーション処理が、存在しないモデルを消すついでにシークレットまで巻き添えで消した可能性が高い。
...
> **クロウ候:** 教訓：設定の自動マイグレーションはシークレットを巻き添えにする可能性がある。アップデート後はトークンの存在を必ず確認すること。

制御を気にしないと大変なことになるな、と思い知った瞬間でした。

## 事務総長の誕生

プロジェクトが増えすぎて、業務フローの管理が追いつかなくなりました。どのプロジェクトがどのフェーズにいるのか、次に何をすべきなのか。把握しきれない。

そこで、フロー管理専用のエージェント「事務総長」（flow-manager）を作りました。この話は長くなるので、[別記事](https://zenn.dev/imudak/articles/ai-agent-secretary-general)に詳しく書いています。
