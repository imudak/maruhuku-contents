...
引数の`-u root`がポイントです。これを省略すると、ユーザーセッションのない状態ではWSLの起動に失敗することがあります。

#### 補足：-u root なしで動作させる方法

WSL側で`loginctl enable-linger`を有効にすると、`-u root`なしでもユーザーサービスの起動が可能になる場合があります。

```bash
sudo loginctl enable-linger ユーザー名
```

この設定により、ユーザーがログインしていなくても `systemctl --user` で管理されるサービス（OpenClawなど）が起動します。

設定後はタスクスケジューラの引数を空にしても動作しますが、環境によって挙動が異なる可能性もあるため、自分は`-u root`を指定しています。
...
Tailscaleはインストール時にスタートアップへ登録されるため、PC起動時に自動で接続されます。タスクバー右下のシステムトレイにTailscaleのアイコンがあれば起動しています。

もし自動起動していない場合は、Tailscaleアプリの設定で「Run at login」にチェックを入れてください。

## 補足：同一ネットワーク内でも同じ方法で接続できる

Tailscaleは**外出先からでも自宅内でも同じコマンドで接続できます**。
...
1. **Tailscaleを選択** - DNS設定の変更が不要で導入も簡単
2. **WSL側でSSHとTailscaleをインストール** - systemdで自動起動に設定
3. **タスクスケジューラでWSL自動起動** - `-u root` オプション、または `loginctl enable-linger` を活用
