...
引数に `-u root` を指定するのがポイントです。これがないと、ユーザーセッションがない状態ではWSLが起動しないことがあります。

#### 補足：-u root なしで動作させる方法

WSL側で `loginctl enable-linger` を有効にすると、`-u root` なしでもユーザーサービスが起動する場合があります。
...
設定後はタスクスケジューラの引数を空にしても動作しますが、環境によって挙動が異なる可能性があるため、確実に動作させたい場合は `-u root` の指定を推奨します。
