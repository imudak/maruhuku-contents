...
記事の冒頭には、読者の共感や笑いを引き出す **ちょっとしたネタ** を入れる。
抽象的な問題提起ではなく、具体的なエピソードや会話から始める。

**良い例:**
> AIエージェントに「MUSUBIで開発して」と言ったら、「ああMUSUBIね、はいはいわかってますよ」という顔をして——わかっていなかった。

**避ける例:**
> 仕様管理は重要です。今回はMUSUBIの運用改善について書きます。

### 実体験ベース

- 実際にやったことを書く（「〜すべきです」ではなく「〜した」）
- 失敗談・発見・驚きを率直に書く
- AIエージェントとの実際のやり取りのニュアンスを活かす

### トーンと文体

- ですます調を基本とする
- AIっぽい定型文は排除（「以下に示します」「〜について解説します」等）
- 適度にカジュアル（技術ブログであって論文ではない）
...
- **実体験が起点の記事は、実体験を冒頭に置く**（背景知識→実体験の教科書型ではなく、実体験→調査→背景の発見型）
- **視点を最後まで一貫させる**（「調べてもらった」で始めたら最後まで他者に調べさせた視点）
- **図解が必要な話にはMermaid図を入れる**（アーキテクチャ、攻撃フロー、Before/After等）
- **対策には「何から守るか」を書く**（手順だけでなく、防ぐ攻撃を明記）
- **同じ趣旨を2箇所で述べない**（本文中の重複を排除、まとめでの要約はOK）
- **末尾の定型文は不要**（「この記事は〜を元に書いています」等）
- **調査データは `.research/` に保存する**（記事に使わなかった情報の再利用用）
